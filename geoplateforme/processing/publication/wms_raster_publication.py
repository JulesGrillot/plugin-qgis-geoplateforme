...
data_type = "WMS-RASTER"


class WmsRasterPublicationAlgorithm(QgsProcessingAlgorithm):
    DATASTORE = "DATASTORE"
    ...
    FEATURE_INFO_SERVER_URL = "FEATURE_INFO_SERVER_URL"
    ...
    def createInstance(self):
        return WmsRasterPublicationAlgorithm()

    def name(self):
        return "wms_raster_publish"

    def displayName(self):
        return self.tr("Publication service WMS Raster")
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                name=self.INTERPOLATION,
                description=self.tr("Interpolation."),
                options=self.INTERPOLATION_ENUM,
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                name=self.BOTTOM_RESOLUTION,
                description=self.tr("Résolution minimale de la couche."),
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                name=self.TOP_RESOLUTION,
                description=self.tr("Résolution maximale de la couche."),
                optional=True,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterString(
                name=self.FEATURE_INFO_SERVER_URL,
                description=self.tr(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        style_str = self.parameterAsString(parameters, self.STYLES, context)
        style_ids = []
        if style_str:
            style_ids = style_str.split(",")
        ...
        if len(style_ids) != 0:
            type_infos["styles"] = style_ids

        bottom_resolution = self.parameterAsDouble(
            parameters, self.BOTTOM_RESOLUTION, context
        )
        if bottom_resolution != 0.0:
            type_infos["bottom_resolution"] = bottom_resolution

        top_resolution = self.parameterAsDouble(
            parameters, self.TOP_RESOLUTION, context
        )
        if top_resolution != 0.0:
            type_infos["top_resolution"] = top_resolution
        server_url = self.parameterAsString(
            parameters, self.FEATURE_INFO_SERVER_URL, context
        )
        if server_url:
            type_infos["getfeatureinfo"] = {
                "server_url": self.parameterAsString(
                    parameters, self.FEATURE_INFO_SERVER_URL, context
                )
            }
