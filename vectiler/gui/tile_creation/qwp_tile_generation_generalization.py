...
class TileGenerationGeneralizationPageWizard(QWizardPage):
    def __init__(self, qwp_tile_generation_edition: TileGenerationEditionPageWizard, parent=None):
        """
        ...
        uic.loadUi(os.path.join(os.path.dirname(__file__), "qwp_tile_generation_generalization.ui"), self)
        ...
        self.tippecanoe_options = {
            ...
            'keep_nodes':
                {'name': 'keep_nodes',
                 ...
                 'explain': self.tr("Toutes les formes sont simplifiées et les nœuds du réseau sont conservés."),
                 'value': '-pn –S15'},
            'delete_smallest':
            ...
            'keep_cover':
                {'name': 'keep_cover',
                 ...
                 'explain': self.tr("Les formes sont simplifiées en conservant une couverture du territoire."),
                 'value': '-aL –D8 –S15'},
            'keep_densest_delete_smallest':
                {'name': 'keep_densest_delete_smallest',
                 'title': self.tr("Sélection de données surfaciques"),
                 'explain': self.tr("Les données les plus représentatives sont conservées et les "
            ...
            'keep_shared_edges':
                {'name': 'keep_shared_edges',
                 ...
                 'explain': self.tr("Les formes sont simplifiées en conservant les limites partagées entre deux "
                                    "surfaces."),
                 'value': '-ab –S20'},
    ...
    def _add_tippecanoe_radiobuttons(self):
        ...
        for key, option in self.tippecanoe_options.items():
            ...
            image_path = DIR_PLUGIN_ROOT / 'resources' / 'images' / 'tippecanoe' / f'{option["name"]}_merged.jpg'
